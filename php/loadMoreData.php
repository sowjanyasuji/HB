...
// $sql="select * from resume_status inner join c_log on c_log.cl_id=resume_status.cv_can_id inner join basic_info on basic_info.b_uid=resume_status.cv_can_id inner join edu_qf on edu_qf.e_uid=resume_status.cv_can_id where resume_status.cv_jb_id = '$jobid' and resume_status.jb_applied ='1' and resume_status.cv_id >'".$_POST['lstId']."' order by resume_status.cv_id limit 1";

$sql="select * from resume_status inner join c_log on c_log.cl_id=resume_status.cv_can_id inner join basic_info on basic_info.b_uid=resume_status.cv_can_id inner join edu_qf on edu_qf.e_uid=resume_status.cv_can_id where resume_status.cv_jb_id = '$jobid' and resume_status.cv_id >'$lid' order by resume_status.cv_id limit 1";
