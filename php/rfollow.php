...
$fstatus = $_POST['fst'];
// echo $fstatus;
if(isset($rid,$fstatus)){
  if($fstatus=='1'){
    $sel= "SELECT * FROM `rec_follow` WHERE `rec_id`='$rid' and `can_id`='$can_id'";
    // echo $sel;
    $result = $connect->query($sel);
        ...
        if ($result->num_rows > 0) {
          while($row = $result->fetch_assoc()) {
              ...
              // echo $fs;
              if($fs == '0'){
                $sql1 ="UPDATE `rec_follow` SET `f_status`='1' where `rec_id`='$rid' and `can_id`='$can_id'";
                  // echo $sql1;
                  ...
                  $sql1 ="UPDATE `rec_follow` SET `f_status`='0' where `rec_id`='$rid' and `can_id`='$can_id'";
                    // echo $sql1;
                    if ($connect->query($sql1) === TRUE) {
                        echo "unfollow";
                    }
              ...
              $sql="INSERT INTO `rec_follow`(`rec_id`,`can_id`,`f_status`) VALUES('$rid','$can_id','$fstatus')";
                    // echo $sql;
